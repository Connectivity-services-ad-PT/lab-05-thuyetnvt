...
- [x] **Newman test:** đã chạy `npm run test:compose` thành công và báo cáo HTML/XML được tạo trong `reports/`.
...
- [ ] **Image tags / registry push (nếu yêu cầu):** nếu giáo viên yêu cầu, bạn đã build image với tag `v0.1.0-<team>` và push lên registry (ghcr.io hoặc Docker Hub). Ghi rõ rằng tag đã được upload.
